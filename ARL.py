...
''' version history
    ...
    v1.2
        change location stopwords.csv / .ttf workdir -> root
...
def Makegraph_Wordcloud_Soynlp(target):
    try:
        if flag_login == 0 or flag_login == None or flag_login == '':
            ...
            cohesion_score = {word:score.cohesion_forward for word, score in words.items()} # force : 여기인가?
            # force join words
            ...
            csv_stopwords = pd.read_csv('stopwords.csv', encoding = 'cp949', skiprows = 0) # with open 변경
            stopwords = list()
            ...
            wordcloud = WordCloud(font_path = 'NanumGothic.ttf', width = 500, height = 500, background_color = 'white').generate_from_frequencies(words_dict_soynlp)
...
def Makegraph_Wordcloud_Konlpy(target):
    try:
        if flag_login == 0 or flag_login == None or flag_login == '':
            ...
            wordcloud = WordCloud(font_path = 'NanumGothic.ttf', width = 500, height = 500, background_color = 'white').generate_from_frequencies(words_dict_konlpy)
...
def Make_Menu():
    ...
    menu_3.add_command(label = '결과폴더 열기', command = showresult)
